...
- [DESCRIPTION](https://pablogguz.github.io/subincomeR/DESCRIPTION) —
  version, deps, Zenodo URL in description.
- [R/getDOSE.R](https://pablogguz.github.io/subincomeR/R/getDOSE.R) —
  downloads + loads the DOSE CSV.
- [R/getDOSE_geom.r](https://pablogguz.github.io/subincomeR/R/getDOSE_geom.r)
  — downloads + loads the GADM-DOSE geometries (the ~769 MB `.gpkg`,
  hosted on Dropbox, not Zenodo).
- [R/matchDOSE.R](https://pablogguz.github.io/subincomeR/R/matchDOSE.R)
  — point-to-region matching; uses `tidygeocoder` +
  [`sf::st_join`](https://r-spatial.github.io/sf/reference/st_join.html).
- [R/subincomeR-package.R](https://pablogguz.github.io/subincomeR/R/subincomeR-package.R)
  — package-level docs.
- [vignettes/regional-convergence.Rmd](https://pablogguz.github.io/subincomeR/vignettes/regional-convergence.Rmd)
  — only vignette; loads `dplyr` so `%>%` is fine inside it.
- [data-raw/](https://pablogguz.github.io/subincomeR/data-raw/) —
  scripts for the hex logo and the landing-page map. Not part of the
  installed package; ignored via `.Rbuildignore`.
- [dev/prepare_release.r](https://pablogguz.github.io/subincomeR/dev/prepare_release.r)
  and
  [dev/prepare_CRAN.r](https://pablogguz.github.io/subincomeR/dev/prepare_CRAN.r)
  — the release / CRAN-submission recipe the maintainer follows
  (devtools, win/mac builders, etc.).
- [inst/WORDLIST](https://pablogguz.github.io/subincomeR/inst/WORDLIST)
  —
  [`spelling::spell_check_package()`](https://docs.ropensci.org/spelling//reference/spell_check_package.html)
  allowlist.
- [tests/spelling.R](https://pablogguz.github.io/subincomeR/tests/spelling.R)
  — runs the spell check during `R CMD check`.
...
- CSV filename inside
  [R/getDOSE.R](https://pablogguz.github.io/subincomeR/R/getDOSE.R)
  (e.g. `DOSE_V2.14.csv`) has to match what’s actually hosted on the
  Zenodo record.
...
was changed in v0.5.0 after `sf` stopped re-exporting `%>%` (see
r-spatial/sf#2607), which surfaced as a CRAN NOTE (“no visible global
function definition for ‘%\>%’”).

The minimum R version is therefore `R (>= 4.1.0)`, declared in
`DESCRIPTION`. Vignettes and `data-raw/` scripts can keep `%>%` because
they explicitly [`library(dplyr)`](https://dplyr.tidyverse.org).
...
4.  Update `cran-comments.md` describing what changed and where it was
    tested.
5.  Run `devtools::check(args = c("--no-manual", "--as-cran"))` and aim
    for `0 errors | 0 warnings | 0 notes`.
6.  Run `devtools::check_win_devel()`, `check_win_release()`,
    `check_mac_release()` — all must come back clean.
7.  `urlchecker::url_check()` (URLs in DESCRIPTION/README/Rd must
    resolve).
8.  `usethis::use_spell_check()` — add false positives to
    `inst/WORDLIST`.
9.  `devtools::submit_cran()`. This auto-updates `CRAN-SUBMISSION` with
    the version + commit SHA, so don’t hand-edit that file.

`CRAN-SUBMISSION` is a stamp of the *last* submission, not a config
file.
...
- **No writes to the user’s home directory.**
  [`getDOSE()`](https://pablogguz.github.io/subincomeR/reference/getDOSE.md)
  and
  [`getDOSE_geom()`](https://pablogguz.github.io/subincomeR/reference/getDOSE_geom.md)
  default to [`tempdir()`](https://rdrr.io/r/base/tempfile.html); only
  write elsewhere if the user passes `path =`. CRAN rejected an earlier
  version for writing to `~` by default — don’t regress this.
- **Large download confirmation.**
  [`getDOSE_geom()`](https://pablogguz.github.io/subincomeR/reference/getDOSE_geom.md)
  prompts via [`utils::menu()`](https://rdrr.io/r/utils/menu.html) in
  interactive sessions because the file is ~769 MB. Non-interactive
  sessions need `download = TRUE` to proceed.
